...
import { chunkText, mapRampartSpans, mergeSpans, reassembleSpans, supplementSpans } from './index'
...
describe('redaction policy', () => {
  it('maps Rampart labels to Obiter categories', () => {
    ...
    const spans = mapRampartSpans({
      text: legalText,
      spans: [{ start, end: start + 'Jane Smith'.length, label: 'GIVEN_NAME', score: 0.9 }],
    })
  ...
  it('fails loudly for unknown Rampart labels', () => {
    expect(() => mapRampartSpans({ text: legalText, spans: [{ start: 0, end: 4, label: 'UNKNOWN' }] })).toThrow(
  ...
  it('deduplicates overlaps with Rampart winning', () => {
    const rampart: RedactionSpan = {
      id: 'span_r', start: 0, end: 10, text: 'Jane Smith', category: 'person_name', source: 'rampart_model', confidence: 'high', suggestion: 'redact',
    }
    const supplement: RedactionSpan = {
      id: 'span_s', start: 5, end: 10, text: 'Smith', category: 'organisation_name', source: 'uk_supplement', confidence: 'low', suggestion: 'keep',
    }
    expect(mergeSpans([rampart], [supplement])).toEqual([rampart])
  })

  it('preserves non-overlapping spans', () => {
    const rampart = mapRampartSpans({ text: legalText, spans: [{ start: 0, end: 10, label: 'GIVEN_NAME' }] })
    const supplement = supplementSpans(legalText).filter((span) => span.category === 'national_insurance')
  ...
  it('chunks text and reassembles offsets', () => {
    ...
    const spans = reassembleSpans([{ chunkOffset: chunk!.startOffset, spans: [{ id: 'span', start: localStart, end: localStart + target.length, text: target, category: 'case_reference', source: 'uk_supplement', confidence: 'medium', suggestion: 'keep' }] }])
