...
- Access control: all endpoints require an authenticated, organisation-scoped user. Ordinary standalone-run reads and mutations require `created_by`; linked runs require active matter view/edit access. The deleted-run audit exception is known-id, organisation-scoped, and owner/admin-only after the live lookup misses. Follow the existing `requireUser()` pattern with org-scoping.
