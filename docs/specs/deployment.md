...
- Node 22 slim base, corepack-enabled pnpm, monorepo build via `pnpm --filter @obiter/api deploy` (or fetch + prune pattern). Copy the repo-root `.npmrc` before the install step so the image inherits the CPU-only ONNX Runtime setting (see the web Dockerfile note above); a GPU host may override it with `ONNXRUNTIME_NODE_INSTALL_CUDA=v12`.
...
- **Repo-root `.npmrc`** — install settings that the lockfile install depends on. It currently carries one: `onnxruntime-node-install-cuda=skip`, because `onnxruntime-node`'s postinstall would otherwise fetch the optional CUDA and TensorRT execution providers (onnxruntime-linux-x64-gpu, ~343 MB unpacked) on Linux x64. Every deployment runs detection on CPU, so the download is dead weight in every cold install, CI cache miss and image layer. **Any Dockerfile that runs `pnpm install` must copy the repo-root `.npmrc` into its build context** (`apps/web/Dockerfile` does; the API image below must, when it lands) — without it the setting is simply absent and the download returns. A GPU host overrides per install with `ONNXRUNTIME_NODE_INSTALL_CUDA=v12 pnpm install`; `pnpm-workspace.yaml` keeps `onnxruntime-node` in `onlyBuiltDependencies` either way, because the skip is about which providers are fetched, not about whether the package's CPU libraries are placed.
