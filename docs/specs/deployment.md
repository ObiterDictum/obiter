...
- Node 22 slim base, corepack-enabled pnpm, monorepo build via `pnpm --filter @obiter/api deploy` (or fetch + prune pattern). Copy the repo-root `.npmrc` into the stage before the deploy step so the image inherits the CPU-only ONNX Runtime setting (see the web Dockerfile note above); a GPU host may override it with `ONNXRUNTIME_NODE_INSTALL_CUDA=v12`, and must then run `pnpm rebuild onnxruntime-node` when the build's pnpm store already holds a CPU-only postinstall result.
