...
- **Tailwind v4 across a workspace package** (utility classes authored in `@obiter/ui` must compile in `apps/web`/`apps/desktop`). Early spike: confirm `@source`/content config compiles classes from a dependency. Fallback: ship precompiled CSS from `@obiter/ui`. Resolved before building primitives on top of it.
