...
- When Ormont API, the legacy `atlas-ingestor` service, and Meilisearch run on the same Dokploy server/network, use the internal service hostname for `MEILISEARCH_HOST`, such as `http://<meilisearch-service-name>:7700`.
...
- Direct browser-to-Meilisearch access through `search.ormont.tech` is allowed only for deliberately public search surfaces and must use a search-only key scoped to public Search indexes.
