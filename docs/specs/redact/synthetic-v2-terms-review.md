...
Claude benchmark generation and QA route through OpenRouter's OpenAI-compatible API because direct Anthropic API access is unavailable. The selected OpenRouter model slugs are explicit environment configuration, and final manifests record the returned model IDs. Direct Anthropic Batch API discounts and prompt caching are not used.
