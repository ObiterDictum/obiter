...
Before the tournament, run `bun run synthetic-v2:smoke` with the same explicit network, terms, credential, judge-provider, judge-model, pricing, private-root, and spend-cap environment used for generation. Set each of `SYNTHETIC_V2_PRIMARY_JUDGE_PROVIDER` and `SYNTHETIC_V2_ADJUDICATOR_PROVIDER` explicitly to `openrouter`, `zai`, or `opencode-go`, alongside its corresponding `*_MODEL`. Z.ai uses `ZAI_API_KEY` against the general API endpoint and requires `OBITER_ZAI_GENERAL_API_CONFIRMED=1`; a GLM Coding Plan key is not eligible because that subscription is restricted to supported coding tools. OpenCode Go uses `OPENCODE_GO_API_KEY`, requires `OBITER_OPENCODE_GO_TERMS_CONFIRMED=1`, and permits only reviewed model/endpoint mappings.

The default `connectivity` smoke profile submits one fixed, 300-word standard diagnostic specification. It proves provider plumbing only and does **not** qualify tournament execution. Before every tournament provider/model configuration, remove `SYNTHETIC_V2_SMOKE_CANDIDATE` and run `bun run synthetic-v2:canary` across all reviewed candidates. This profile preserves the complete first tournament specification, including its original length, categories, matrix cells, and hard negatives. A successful full-candidate run writes a hash-bound receipt under `tournament-canaries/`; the tournament refuses paid calls unless the private root contains a matching receipt for the current specification and judge configuration.

Both profiles permit no source regeneration and write immutable artifacts under the external private-corpus root. Tournament-canary qualification additionally requires every writer, annotator, and judge response to satisfy its structural contract on the first attempt: validation retries or pipeline repair may preserve diagnostic evidence but cannot create a receipt. Candidate quality is evaluated separately: a first-source omission or other specification rejection is valid tournament evidence, does not trigger annotation invention, and may qualify the provider path only after both independent judges have been exercised successfully on that source. After a partial connectivity failure, `SYNTHETIC_V2_SMOKE_CANDIDATE` may select one reviewed candidate so successful paid paths are not replayed, but selected-candidate evidence cannot create a tournament receipt. Smoke artifacts are diagnostic only: they are not corpus partitions and never enter tournament, training, development, or benchmark registries. The command defaults to a GBP 1 worst-case reservation cap. The more conservative tournament-canary estimate may exceed that default; set `SYNTHETIC_V2_SMOKE_MAX_GBP` only to a separately reviewed and approved estimate.

The reviewed tournament candidates use OpenRouter `anthropic/claude-sonnet-4.6` for independent annotation after Gemini 3.6 Flash failed repeated exact-spec structural conformance checks. Annotators select zero-based, end-exclusive ranges from a deterministic immutable source-token list; local code constructs exact source quotes and UTF-16 offsets, so providers never supply quotes, occurrences, or character offsets. Tournament qualification and execution require the reviewed pairing `SYNTHETIC_V2_PRIMARY_JUDGE_PROVIDER=openrouter`, `SYNTHETIC_V2_PRIMARY_JUDGE_MODEL=openai/gpt-4.1`, `SYNTHETIC_V2_ADJUDICATOR_PROVIDER=opencode-go`, and `SYNTHETIC_V2_ADJUDICATOR_MODEL=grok-4.5`. GPT-4.1 uses OpenRouter strict JSON Schema without hidden reasoning-token consumption. Judges review indexed proposed spans using keep/remove/recategorize decisions, report only genuinely missing exact-quote spans, and return one result per hard-negative assertion; local code constructs and validates the corrected reference, and primary/dispute disagreement requires human adjudication. This bounded review contract replaces unreliable full-reference regeneration. GPT-5.4 Mini is prohibited after two exact-spec runs exhausted the full output budget without returning a complete structured answer, and the unqualified Qwen 3.7 Max OpenCode Go Anthropic-compatible route is prohibited after exact-spec transport rejection. Other allowlisted judge routes remain available for connectivity diagnostics but cannot authorize or assemble a tournament. Provider-qualified pricing entries are preferred so subscription quota is recorded at its published notional rate.
