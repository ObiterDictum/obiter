...
describe('POST /api/documents/:id/edit on a run with a page break', () => {
  it('saves the typed text and its emphasis with the break intact', async () => {
    ...
    const response = await app.request(
      ...
      composedRequest('abXYcd', { from: 2, to: 6 }),
    )
  ...
  it('persists nothing when the same composed payload is invalid', async () => {
    ...
    const response = await app.request(
      '/api/documents/doc_1/edit',
      composedRequest('abXYcd', { from: 5, to: 2 }),
    )
...
function composedRequest(text: string, range: { from: number; to: number }) {
  return {
    method: 'POST',
    ...
    body: JSON.stringify({
      baseVersionId: 'ver_1',
      operations: [
        { type: 'replace_run_text', runId, text },
        {
