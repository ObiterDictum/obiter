import type { RedactionRunRecord, RedactionAuditLogEntry } from './redaction-database'

export const AUDIT_REPORT_VERSION = 'obiter.redaction-audit/v1'

export function buildAuditReport(run: RedactionRunRecord, auditLog: RedactionAuditLogEntry[]) {
  const finalized = [...auditLog].reverse().find((entry) => entry.action === 'redaction.finalize')
  return {
    ...
    auditLog,
    reviewerInfo: finalized ? { userId: finalized.userId, reviewedAt: finalized.timestamp } : null,
  }
}
...
export function renderAuditMarkdown(report: ReturnType<typeof buildAuditReport>) {
