...
export function exportReviewedRuns(runs: InputRun[], includePartial = false) {
  return runs
    .filter(
      (run) =>
        run.status === 'finalized' && (run.fullyReviewed || includePartial),
    )
    .map((run) => {
...
async function main() {
  const [
    inputPath,
    outputPath = 'data/evals/redact/exported_training_data.jsonl',
    flag,
  ] = process.argv.slice(2)
  if (!inputPath)
    throw new Error(
      'Usage: tsx scripts/export-training-data.ts <reviewed-runs.json> [output.jsonl] [--include-partial]',
    )
  let runs: InputRun[]
  try {
