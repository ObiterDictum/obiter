...
Do not label neutral citations, statutes, court names, hearing dates, procedural deadlines, damages figures, company registration numbers, or generic role references. Do not alter, paraphrase, add, remove, reorder, or correct document text. Offsets are optional cross-checks, zero-based UTF-16 indices with end exclusive; quote plus occurrence is authoritative. Exhaustively label every in-scope category in the whole document, not only requested examples. Requested categories are minimum generation quotas, not a limit on annotation. Before responding, silently verify that every required category appears at least once, every span selects the intended source substring, and spans do not overlap.`
