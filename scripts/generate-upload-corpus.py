...
def main():
    ...
    for name, build in BUILDERS.items():
        ...
        doc = build()
        # python-docx stamps dc:description with producer graffiti
        # ("generated by python-docx") that the extraction-coverage guard
        # rightly refuses as unexamined docProps text. Its property setter
        # ignores empty values, so drop the element directly. Fixtures must
        # be clean ordinary documents; real docProps content is covered by
        # crafted unit-test packages instead.
        _core = doc.core_properties._element  # noqa: SLF001 (script surgery)
        _desc = _core.find(
