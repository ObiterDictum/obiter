...
An independent Claude Haiku model routed through OpenRouter will review every benchmark document against the plain text and proposed spans. A second independent judge handles disagreement, uncertainty, protected-person examples, and hard-negative conflicts; a document is regenerated unless it receives automated agreement. `supplementSpans` is run over every document as an additional mechanical miss signal. Human review is limited to every disputed item and a stratified 15–20% benchmark audit. The final committed QA report will state sample sizes, agreement, flagged cells, regeneration counts, and audit outcomes.
