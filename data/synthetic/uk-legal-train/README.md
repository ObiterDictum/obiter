...
This placeholder documents the product boundary only. Do not add corpus documents, raw provider output, review annotations, or run logs here. Run `bun run bench:guard` before using any private-corpus export for fine-tuning; a non-zero result means the export overlaps the public benchmark and must not be used.
